...
class Solution:
    ...
    def getIntersectionNode_set(self, headA: ListNode, headB: ListNode) -> ListNode:
        # APPORACH 2: Using a set
    ...
    def getIntersectionNode_brute_force(self, headA: ListNode, headB: ListNode) -> ListNode:
        # APPROACH 1: For each node in list A, traverse ojver list B and check whether or not the node is present in list B.
        while headA:
            pB = headB
            while pB:
                if headA == pB:
