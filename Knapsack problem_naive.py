def find_knapsack_naive_approach(capacity, weights, values, n):
	"""
	The implementation of knapsack problem 0/1 bounded using naive approach
	For a specific item, we can either take it and add its weight to the total or do not  include it
	"""
	...
	if weights[n - 1] <= capacity:
		return max(
			values[n - 1] + find_knapsack_naive_approach(capacity - weights[n - 1], weights, values, n - 1),
			find_knapsack_naive_approach(capacity, weights, values, n - 1)
		)
	# Item can't be added in our knapsack
	# if it's weight is greater than the capacity
	else:
		return find_knapsack_naive_approach(capacity, weights, values, n - 1)


# time: O(2^n), n is the total of items, because we have two possible choices every time, either include the item or not
...
def main():
	...
	for i in range(len(values)):
		...
		result = find_knapsack_naive_approach(capacity[i], weights[i], values[i], n[i])
